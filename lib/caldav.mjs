...
export class CalDavClient {
  ...
  /**
   * Get deduplicated list of calendars as {name, path} pairs.
   * kind: "event" → VEVENT 集合（日历），"todo" → VTODO 集合（任务列表）。
   */
  async getAllCalendars(kind = "event") {
    const map = await this.getCalendarMap(kind);
    const seen = new Set();
    const out = [];
  ...
  /**
   * Get full calendar path for a named calendar, routed by kind:
   * - "event" → VEVENT 日历集合（如 work → 工作日历）
   * - "todo"  → VTODO 任务列表集合（如 work → 工作任务列表）
   * 显式 config.calendars 覆盖优先（在 getCalendarMaps 中统一注入）。
   */
  async getCalendarPath(calendar, kind = "event") {
    const map = await this.getCalendarMap(kind);
    const path = map[calendar];
    if (path) return path;
    if (kind === "todo") {
      return map["inbox"] || map["待办"] || Object.values(map)[0] || "";
    }
    return map["home"] || map["家庭"] || Object.values(map)[0] || "";
  }

  /**
   * Get lazy-initialized { eventMap, todoMap } for the current user.
   * discover 时按 supported-calendar-component-set 识别集合类型：
   * VEVENT 集合进 eventMap（日程），VTODO 集合进 todoMap（任务列表），
   * 同名集合不再互相覆盖——event 与 todo 各归其位。
   */
  async getCalendarMaps() {
    ...
    for (const cal of calendars) {
      ...
      if (cal.types.includes("VEVENT")) put(eventMap);
      if (cal.types.includes("VTODO")) put(todoMap);
    }
    ...
    this.calendarCache = { eventMap, todoMap };
    return this.calendarCache;
  }

  /** Get single name→path map for a kind ("event" | "todo"). */
  async getCalendarMap(kind = "event") {
    const maps = await this.getCalendarMaps();
    return kind === "todo" ? maps.todoMap : maps.eventMap;
  }
  ...
  /**
   * Extract supported component types (VEVENT/VTODO/...) from a response block.
   * 群晖前缀不固定（C: / C1: / c:），且集合可能自闭合——宽松匹配。
   */
  extractCompTypes(responseXml) {
    const m = /<[\w-]+:?supported-calendar-component-set[^>]*>([\s\S]*?)<\/[\w-]+:?supported-calendar-component-set>/i.exec(responseXml);
    const text = m ? m[1] : "";
    const types = [];
    const re = /name="([A-Z]+)"/g;
    let x;
    while ((x = re.exec(text)) !== null) types.push(x[1]);
    return types;
  }
...
<d:propfind xmlns:d="DAV:" xmlns:cs="http://calendarserver.org/ns/" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    ...
    for (const resp of responses) {
      ...
      // 群晖用任意前缀（C: / C1: / c:）且可能自闭合，正则须宽松匹配
      const types = this.extractCompTypes(resp);
      // 服务器未返回类型时保守处理：按两者都支持（保持旧行为）
      if (types.length === 0) types.push("VEVENT", "VTODO");
